...
{
	Super::BeginPlay();
	FileWriter = IFileManager::Get().CreateFileWriter( *(FPaths::ProjectSavedDir() / "CapturedVideo.h264"));
...
void AActorVideoEncoder::StopWrite()
{
	if(FileWriter != nullptr)
	{
		FileWriter->Close();
		FileWriter = nullptr;
		UVideoCaptureComponent::EncodeH264ToMP4(*(FPaths::ProjectSavedDir() / "CapturedVideo.h264"),
			*(FPaths::ProjectSavedDir() / "CapturedVideo.mp4"));
	}
}
...
void AActorVideoEncoder::OnEncodedVideoFrame(uint32 LayerIndex, const AVEncoder::FVideoEncoderInputFrame* Frame, const AVEncoder::FCodecPacket& Packet)
{
	...
	if(FileWriter != nullptr)
	{
		FileWriter->Serialize(const_cast<uint8*>(Packet.Data), Packet.DataSize);
	}
